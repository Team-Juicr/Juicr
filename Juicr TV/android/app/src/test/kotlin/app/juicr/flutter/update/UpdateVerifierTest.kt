...
class UpdateVerifierTest {
    ...
    fun `checksum mismatch fails before archive inspection`() = withApk("candidate") { file ->
        ...
        val verifier = UpdateVerifier("tv", PACKAGE_NAME, inspector)
    ...
    @Test
    fun `same package and signer from android lane is rejected`() = withApk("candidate") { file ->
        val verifier = verifier(file, archive = archiveIdentity().copy(lane = "android"))
